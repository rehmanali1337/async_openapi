...
class BaseModel:
    def __init__(self, api_key: str, model: str, base_endpoint: str, temperature: float = 1.0) -> None:
    ...
    async def _request(self, data: GeneralDict, method: str = "POST") -> GeneralDict:
        async with self._aiohtt_client.request(
            method=method, url=self._completions_endpoint, headers=self.get_headers(), data=json.dumps(data)
        ) as resp:
            await resp.read()
